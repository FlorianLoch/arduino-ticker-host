...
(async () => {
  if (!config.secret) {
    console.log(
      "=> No secret for communication provided. Please add 'secret' to your configuration."
    );
    return;
  }
    ...
    for (file of pluginEntryFiles) {
      try {
        ...
        await plugin.onLoad(
          ...
          app,
          (message) => {
            console.log(c.green(`=> '${pluginId}' sends: '${message}'`));
            udpService.broadcast({
              type: "msg",
              payload: message,
            });
          }
        ...
        console.log(c.red(`=> Failed loading plugin from '${file}': `), err);
      }
    }
  }
})();
